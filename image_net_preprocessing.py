...
def main():
    ...
    for folder_name in folder_name_list:
        ...
        for image in images[folder_name]:
            #-----------------------------------Changed by Joseph from the original code-------------------------
            ...
            top = int((height-smallest)/2)
            bottom = int(((height-smallest)/2)+smallest)
            left = int((width-smallest)/2)
            right = int(((width-smallest)/2)+smallest)
            cropped_image = image[top:bottom, left:right]
            index = remove_image_from_list(images[folder_name], image)
            images[folder_name].insert(index, cropped_image)
            #-------------------------------------------End of change--------------------------------------------

        # Resizing the images to the same resolution.
        for image in images[folder_name]:
            if image.shape[:2] != (375, 375): #modified dimensions to make square
                resized_image = cv2.resize(image, (375, 375))
                index = remove_image_from_list(images[folder_name], image)
